...
class DataRecordForm(tk.Frame):
    ...
    def __init__(self, parent, *args, **kwargs):
        ...
        self.inputs['Humidity'].grid(row=0, column=0, padx=20,pady=20)
        self.inputs['Humidity'] = LabelInput(
            environmentinfo, "Humidity (g/m³)",
            input_class=tk.Spinbox, input_var=tk.DoubleVar(),
            input_args={"from_": 0.5, "to": 52.0, "increment": .01})
        self.inputs['Humidity'].grid(row=0, column=1, padx=20,pady=20)
        self.inputs['Humidity'] = LabelInput(
            environmentinfo, "Humidity (g/m³)",
            input_class=tk.Spinbox, input_var=tk.DoubleVar(),
            input_args={"from_": 0.5, "to": 52.0, "increment": .01})
        self.inputs['Humidity'].grid(row=0, column=2, padx=20,pady=20)
        ...
        self.inputs['Plants'].grid(row=0, column=0, padx=20,pady=20)
        self.inputs['Blossoms'] = LabelInput(
            plantinfo, "Blossoms",
            input_class=tk.Spinbox,
            input_var=tk.IntVar(),
            input_args={"from_": 0, "to": 1000})
        self.inputs['Blossoms'].grid(row=0, column=1, padx=20,pady=20)
...
class Application(tk.Tk):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.recordform.grid(row=1, padx=50)

        self.savebutton = ttk.Button(self, text="Save", command=self.on_save)
        self.savebutton.grid(sticky=tk.E, row=2, padx=50)

        self.status = tk.StringVar()
        self.statusbar = ttk.Label(self, textvariable=self.status)
        self.statusbar.grid(sticky=(tk.W + tk.E), row=3, padx=50)
